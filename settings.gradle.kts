...
rootDir.listFiles()?.filter {
    ...
    it.listFiles()?.filter { pit ->
        pit.isDirectory
            && (File(pit, "build.gradle.kts").exists())
    }?.forEach { pit ->
        println(pit.name)
...
include("anthill-infrastructure:anthill-infrastructure-doc")
findProject(":anthill-infrastructure:anthill-infrastructure-doc")?.name = "anthill-infrastructure-doc"
