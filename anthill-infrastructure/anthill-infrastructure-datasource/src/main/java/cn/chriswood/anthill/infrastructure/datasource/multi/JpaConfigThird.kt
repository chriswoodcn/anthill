...
@EnableJpaRepositories(
    ...
    entityManagerFactoryRef = Constants.THIRD + "EntityManagerFactory",
    //配置事务管理  transactionManager
    transactionManagerRef = Constants.THIRD + "TransactionManager",
)
@ConditionalOnProperty(
    prefix = "anthill.jpa.multi.third",
    name = ["enabled"],
    havingValue = "true",
)
@EnableConfigurationProperties(JpaDataSourceProperties::class)
class JpaConfigThird(
    ...
    @Bean(name = [Constants.THIRD])
    @Primary
    fun dataSource(): DataSource {
        val dataSource = HikariDataSource()
        dataSource.driverClassName = properties.multi[Constants.THIRD]!!.driver
        dataSource.jdbcUrl = properties.multi[Constants.THIRD]!!.url
        dataSource.username = properties.multi[Constants.THIRD]!!.username
        dataSource.password = properties.multi[Constants.THIRD]!!.password
        dataSource.poolName = String.format("HikariPool-%s", Constants.THIRD)
        dataSource.isReadOnly = false
        dataSource.connectionTestQuery = properties.multi[Constants.THIRD]!!.query
        return dataSource
    }

    @Bean(name = [Constants.THIRD + "EntityManager"])
    fun entityManager(
        @Qualifier(Constants.THIRD)
    ...
    @Bean(name = [Constants.THIRD + "EntityManagerFactory"])
    fun entityManagerFactory(
        ...
        return builder
            ...
            .packages(properties.multi[Constants.THIRD]!!.packageScan)
            // Spring会将EntityManagerFactory注入到Repository之中.有了 EntityManagerFactory之后,
            // Repository就能用它来创建 EntityManager 了,然后 EntityManager 就可以针对数据库执行操作
            .persistenceUnit(Constants.THIRD + "PersistenceUnit")
            .build()
    }
...
// The only reason to specify this property is if you’re using a custom user-written Dialect class.
    ...
    @Bean(name = [Constants.THIRD + "TransactionManager"])
